...
```
# Here it is important to note that some fields we are choosing to store. Why would we store some and not others? 
# Well the reason is because assuming your _source is not disabled you can retrieve all of your data no matter what. 
# Elasticsearch will by default put your field in the _source. But if you enforce storing to true it will pull the
# field from the index itself rather than the _source. This is only used for large data sets where the cost of
# parsing the source is high. I'm only using this as an example for you to let you know it's possible.
